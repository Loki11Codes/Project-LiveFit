...
describe('Knowledge API Route', () => {
  ...
  describe('GET', () => {
    ...
    it('returns knowledge entries on success', async () => {
      ...
      vi.mocked(prisma.userKnowledge.findMany).mockResolvedValue([{ id: '1', userId: 'u1', key: 'k', value: 'v', updatedAt: new Date() }]);
      const res = await GET();
  ...
  describe('POST', () => {
    ...
    it('upserts knowledge entry on success', async () => {
      vi.mocked(getServerSession).mockResolvedValue({ user: { id: 'u1' } } as Session);
      vi.mocked(prisma.userKnowledge.upsert).mockResolvedValue({ id: '1', userId: 'u1', key: 'k', value: 'v', updatedAt: new Date() });
